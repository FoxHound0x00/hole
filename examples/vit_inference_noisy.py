"""
ViT CIFAR-10 Inference with HOLE Analysis - Noisy Data

This script loads a trained ViT model, adds Gaussian noise to input data,
runs inference on a balanced subset of CIFAR-10, extracts embeddings, and performs HOLE analysis.
"""
...
def extract_vit_embeddings(model, dataloader, device, noise_std=0.1):
    """Extract embeddings from all ViT encoder layers separately with noisy inputs."""
    model.eval()
...
def select_balanced_subset(dataset, n_per_class=100):
...
def main():
    """Run ViT inference on noisy balanced CIFAR-10 subset with HOLE analysis."""
    
    # Configuration
    N_PER_CLASS = 100  # 100 images per class (1000 total)
    BATCH_SIZE = 32
    NOISE_STD = 0.1  # Gaussian noise standard deviation
    MODEL_PATH = "../vit_cifar10_finetuned"
    OUTPUT_DIR = "vit_noisy_hole_outputs"
    ...
    print("=== ViT CIFAR-10 Inference with HOLE Analysis (Noisy Data) ===\n")
    print(f"Noise level: Gaussian noise with std={NOISE_STD}\n")
    ...
    # Extract embeddings with noise
    print(f"Extracting ViT embeddings from all layers (with noise std={NOISE_STD})...")
    layer_embeddings, labels = extract_vit_embeddings(model, dataloader, DEVICE, noise_std=NOISE_STD)
    ...
    for layer_idx in range(n_layers):
        ...
        plot_dimensionality_reduction(
            ...
            labels=labels,
            ax=ax,
            title=f"Layer {layer_idx} - PCA (Noisy Data, std={NOISE_STD})",
            point_size=50,
            alpha=0.7
        )
        ...
        fig = blob_viz.plot_pca_with_cluster_hulls(
            ...
            title=f"Layer {layer_idx} - Blob Visualization (Noisy, Threshold: {middle_threshold:.3f})",
        )
        plt.close(fig)
        ...
        visualizer.plot_persistence_diagram(ax=ax1, title=f"Layer {layer_idx} - Persistence Diagram (Noisy)", pts=20)
        plt.tight_layout()
        fig1.savefig(f"{layer_output_dir}/persistence_diagram.png", dpi=300, bbox_inches="tight")
        plt.close(fig1)
        
        fig2, ax2 = plt.subplots(1, 1, figsize=(8, 6))
        visualizer.plot_persistence_barcode(ax=ax2, title=f"Layer {layer_idx} - Persistence Barcode (Noisy)", pts=20)
        plt.tight_layout()
    ...
    print(f"Noise level: Gaussian with std={NOISE_STD}")
