# Model: MLP-Mixer-L/16 (requires timm library: pip install timm)
import os
import torch
...
MODEL_PATH = "../finetune/mixer_l16_cifar10.pth"
OUTPUT_DIR = "hole_outputs/mixer_l16"
...
# Load model
print("Loading finetuned MLP-Mixer-L/16...")
model = timm.create_model('mixer_l16_224.goog_in21k_ft_in1k', pretrained=False, num_classes=10)
...
def hook_fn(module, input, output):
    # For Mixer models, we want the output of the norm layer before head
    activations.append(input[0].detach().cpu().numpy())

# Register hook on the head (classifier)
...
fig = blob_viz.plot_pca_with_cluster_hulls(
    ...
    title=f"MLP-Mixer-L/16 - Threshold: {middle_threshold:.3f}",
)
plt.close(fig)
