# Model: ViT_L_32_Weights.IMAGENET1K_V1
import os
import torch
...
MODEL_PATH = "../finetune/vitl32_cifar10.pth"
OUTPUT_DIR = "hole_outputs/vitl32"
...
# Optional: Filter to subset of classes (e.g., 3 random classes)
# SELECTED_CLASSES = [0, 3, 7]  # Uncomment to use only airplane, cat, horse

# Sample 50 images per class
print(f"Sampling {SAMPLES_PER_CLASS} images per class...")
indices = []
class_counts = {i: 0 for i in range(10)}
# class_counts = {i: 0 for i in SELECTED_CLASSES}  # Uncomment if using SELECTED_CLASSES
for idx, (_, label) in enumerate(test_dataset):
    # if label not in SELECTED_CLASSES: continue  # Uncomment if using SELECTED_CLASSES
...
# Load model
print("Loading finetuned ViT-L/32...")
...
# Hook to extract activations from the layer before final head
activations = []
labels_list = []

def hook_fn(module, input, output):
    # ViT encoder output is already flattened
    activations.append(input[0].detach().cpu().numpy())

# Register hook on the head (to get input to head)
...
fig = blob_viz.plot_pca_with_cluster_hulls(
    ...
    title=f"ViT-L/32 - Threshold: {middle_threshold:.3f}",
)
plt.close(fig)
