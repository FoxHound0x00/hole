"""
ViT CIFAR-10 Inference with HOLE Analysis - Random Subset

This script loads a trained ViT model, runs inference on a random subset
of CIFAR-10, extracts embeddings, and performs HOLE topological analysis.
"""
...
def main():
    """Run ViT inference on random CIFAR-10 subset with HOLE analysis."""
    
    # Configuration
    N_SAMPLES = 200  # Random subset size
    BATCH_SIZE = 32
    MODEL_PATH = "../vit_cifar10_finetuned"
    OUTPUT_DIR = "vit_random_hole_outputs"
    ...
    print("=== ViT CIFAR-10 Inference with HOLE Analysis (Random Subset) ===\n")
    ...
    np.random.seed(42)
    random_indices = np.random.choice(len(test_ds), N_SAMPLES, replace=False)
    subset_ds = Subset(test_ds, random_indices)
    
    print(f"Selected {N_SAMPLES} random samples from test set\n")
