...
This experiment systematically tests all distance metrics across various data structures
to understand how different topological features emerge under different conditions.
...
def compute_all_distance_matrices(X):
    ...
    try:
        ...
        if np.any(np.isinf(geodesic_dist)):
            max_finite = np.nanmax(geodesic_dist[np.isfinite(geodesic_dist)])
...
def generate_all_visualizations(
    ...
    for metric_name, dist_matrix in distance_matrices.items():
        ...
        try:
            ...
            # 4. Scatter hull visualization
            scatter_hull_viz = viz.get_scatter_hull_visualizer(
                figsize=(12, 8), alpha_hull=0.3
            )
            ...
            try:
                results = scatter_hull_viz.analyze_blob_separation(
                    ...
                    condition_name=f"{variant_name}_{metric_name}",
                    layer_name="input",
                    distance_metric=metric_name,
                )
            except Exception as e:
                print(f"        ! Scatter hull analysis failed for {metric_name}: {e}")
