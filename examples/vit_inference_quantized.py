"""
ViT CIFAR-10 Inference with HOLE Analysis - INT8 Quantized Model

This script loads a trained ViT model, applies INT8 dynamic quantization,
...
def extract_vit_embeddings(model, dataloader, device):
    """Extract embeddings from all ViT encoder layers separately from quantized model."""
    model.eval()
    ...
    hooks = []
    # For quantized models, access layers through the original structure
...
def main():
    ...
    print("=== ViT CIFAR-10 Inference with HOLE Analysis (INT8 Quantized Model) ===\n")
    ...
    print(f"Quantized model running on {DEVICE}\n")
    ...
    # Extract embeddings from quantized model
    print("Extracting ViT embeddings from all layers (quantized model)...")
    ...
    for layer_idx in range(n_layers):
        ...
        plot_dimensionality_reduction(
            ...
            labels=labels,
            ax=ax,
            title=f"Layer {layer_idx} - PCA (INT8 Quantized)",
            point_size=50,
            alpha=0.7
        )
        ...
        fig = blob_viz.plot_pca_with_cluster_hulls(
            ...
            title=f"Layer {layer_idx} - Blob Visualization (INT8, Threshold: {middle_threshold:.3f})",
        )
        plt.close(fig)
        ...
        visualizer.plot_persistence_diagram(ax=ax1, title=f"Layer {layer_idx} - Persistence Diagram (INT8)", pts=20)
        plt.tight_layout()
        fig1.savefig(f"{layer_output_dir}/persistence_diagram.png", dpi=300, bbox_inches="tight")
        plt.close(fig1)
        
        fig2, ax2 = plt.subplots(1, 1, figsize=(8, 6))
        visualizer.plot_persistence_barcode(ax=ax2, title=f"Layer {layer_idx} - Persistence Barcode (INT8)", pts=20)
        plt.tight_layout()
    ...
    print("Model: INT8 Dynamically Quantized")
