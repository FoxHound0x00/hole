...
class TestFullWorkflowIntegration:
    ...
    def test_complete_visualization_pipeline(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            viz.plot_dimensionality_reduction(method="pca", ax=axes[1, 0], true_labels=self.labels)
            viz.plot_dimensionality_reduction(method="mds", ax=axes[1, 1], true_labels=self.labels)
            ...
            if evolution and "components_" in evolution and "labels_" in evolution:
                ...
                fig = flow_viz.plot_sankey_flow(
                    evolution,
                    save_path=os.path.join(temp_dir, "sankey_test.png")
                )
                plt.close(fig)
            
            # Verify files were created
            expected_files = ["persistence_test.png", "blob_test.png", "heatmap_test.png"]
    ...
    def test_multiple_distance_metrics_integration(self):
        ...
        for metric in metrics:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                viz = hole.HOLEVisualizer(point_cloud=self.points, distance_metric=metric)
