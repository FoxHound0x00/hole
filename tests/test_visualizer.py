...
class TestHOLEVisualizer:
    ...
    def test_initialization_validation(self):
        ...
        with pytest.raises(ValueError, match="Provide only one"):
            HOLEVisualizer(
                ...
                distance_matrix_input=self.distance_matrix
            )
    ...
    def test_plot_dimensionality_reduction(self, mock_show):
        ...
        viz.plot_dimensionality_reduction(
            method="pca", ax=ax, true_labels=self.labels
        )
        plt.close(fig)
        ...
        fig, ax = plt.subplots()
        viz.plot_dimensionality_reduction(
            method="mds", ax=ax, true_labels=self.labels
        )
        plt.close(fig)
        ...
        fig, ax = plt.subplots()
        viz.plot_dimensionality_reduction(
            method="tsne", ax=ax, true_labels=self.labels
        )
        plt.close(fig)
